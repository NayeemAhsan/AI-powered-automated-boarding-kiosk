...
def validate_name(id_data: list, bp_data: list, manifest_row: pd.Series) -> bool:
    ...
    logger.info(f"Name mismatch: ID({id_first_name} {id_last_name}), BP({bp_first_name} {bp_last_name}), Manifest did not match.")
    return False
...
def validate_dob(id_data: list, manifest_row: pd.Series) -> bool:
    ...
    try:
        ...
        if id_dob_standardized == manifest_dob_standardized:
            logger.info(f"DoB Validation: True. id_dob({id_dob_standardized}), manifest_dob({manifest_dob_standardized})")
            return True
...
def validate_all(id_data, boarding_pass_data, face_results, manifest_df):
    ...
    passenger_info = None  # Initialize passenger_info
    ...
    for index, passenger in manifest_df.iterrows():
        ...
        # Ensure manifest_df is not empty after validation
            if manifest_df.empty:
                ...
                passenger_info = [
                    passenger['First Name'], passenger['Last Name'], passenger['Flight No.'], 
                    passenger['Boarding Time'], passenger['From'], passenger['To'], passenger['Seat'],
                    manifest_df.at[index, 'NameValidation'], 
                    manifest_df.at[index, 'DoBValidation'], 
                    manifest_df.at[index, 'PersonValidation'], 
                    manifest_df.at[index, 'BoardingPassValidation'], 
                    manifest_df.at[index, 'LuggageValidation'], 
                    manifest_df.at[index, 'ValidationStatus']
                ]
...
def get_validation_messages(passenger_info, environment="console"):
    ...
    try:
        ...
        # Unpack passenger_info list into variables
        first_name, last_name, flight_no, boarding_time, origin, destination, seat, ValidationStatus, PersonValidation, NameValidation, DoBValidation, BoardingPassValidation, LuggageValidation = passenger_info

        # Generate message based on the ValidationStatus flag
        if NameValidation and DoBValidation and BoardingPassValidation:
            if PersonValidation and LuggageValidation:
                message = (
            ...
            elif PersonValidation == True and LuggageValidation == False:
                message = (
                    f"Dear {first_name} {last_name},{newline}"
                    f"You are welcome to flight #{flight_no} leaving at {boarding_time}.{newline}"
                    f"From {origin} to {destination}, your seat number is {seat}, and it is confirmed.{newline}"
            ...
            elif PersonValidation == False and LuggageValidation == True:
                message = (
                    f"Dear {first_name} {last_name},{newline}"
                    f"You are welcome to flight #{flight_no} leaving at {boarding_time}.{newline}"
                    f"From {origin} to {destination}, your seat number is {seat}, and it is confirmed.{newline}"
        ...
        elif NameValidation == False or DoBValidation == False:
            message = (
                f"Dear Sir/Madam,{newline}"
                f"Some of the information on your ID card does not match the flight manifest data, so you cannot board the plane.{newline}"
                "Please see a customer service representative."
            )
        elif BoardingPassValidation == False:
            message = (
                f"Dear Sir/Madam,{newline}"
                f"Some of the information in your boarding pass does not match the flight manifest data, so you cannot board the plane.{newline}"
                "Please see a customer service representative."
            )            
        return message
...
if __name__ == "__main__":
    try:
        ...
        # get video faces
        video_file_path = config['video_indexer']['video_path']
